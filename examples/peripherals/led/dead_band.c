...
PrintRunningDots(void)
{
   ROM_UARTprintf(". ");
    ...
    ROM_UARTprintf(". ");
    ROM_SysCtlDelay(ROM_SysCtlClockGet() / 3);
    ROM_UARTprintf("\b\b\b\b\b\b\b\b\b\b");
    ROM_UARTprintf("          ");
    ROM_UARTprintf("\b\b\b\b\b\b\b\b\b\b");
    ROM_SysCtlDelay(ROM_SysCtlClockGet() / 3);
}
...
main(void)
{
    ...
    //
    ROM_UARTprintf("PWM ->\n");
    ROM_UARTprintf("  Module: PWM0\n");
    ROM_UARTprintf("  Pin(s): PD0 and PD1\n");
    ROM_UARTprintf("  Features: Dead-band Generation\n");
    UARTprintf("  Duty Cycle: 25%% on PD0 and 75%% on PD1\n");
    ROM_UARTprintf("  Dead-band Length: 160 cycles on rising and falling edges\n\n");
    ROM_UARTprintf("Generating PWM on PWM0 (PD0) -> ");
    ...
    ROM_GPIOPinConfigure(GPIO_PD1_PWM1);
    ...
    // using.
    //
    ROM_PWMGenPeriodSet(PWM_BASE, PWM_GEN_0, 64000);
    ...
    //
    ROM_PWMPulseWidthSet(PWM_BASE, PWM_OUT_0,
    ...
    // generation.
    //
    ROM_PWMDeadBandEnable(PWM_BASE, PWM_GEN_0, 160, 160);

    //
    // Enable the PWM0 Bit 0 (PD0) and Bit 1 (PD1) output signals.
    //
    ROM_PWMOutputState(PWM_BASE, PWM_OUT_1_BIT | PWM_OUT_0_BIT, true);
